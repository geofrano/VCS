            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $.each(data.items, function(index, article) {
                        cont = cont + 1;
                        $tabla.append("<tr title=\"Si no ha ingresado la ficha del estudiante no podra generar el Informe de Petición Verbal\"><td><input type=\"hidden\" name=\"cc_id_" + cont + "\" id=\"cc_id_" + cont + "\" value=\"" + article.id_cc + "\" >" + article.id_cc + "</td>\n" +
                        ...
                        if (article.cc_estado >= 1) {//Solo si esta en estado 1 (ya se genero la carta comp)
                            $("#" + "div_imprimir_" + cont).css("display", "block");
                        }if (article.cc_estado = "A") {//Solo si esta en estado A (ya se genero la carta comp)
                            $("#" + "div_imprimir_" + cont).css("display", "block");
                        }
...
function imprime(cont) {
    ...
    document.getElementById("frm_ficha").action = ruta + "/Entidad_Externa/Informe_Peticion_Verbal/imprime_peticion_verbal.jsp";
