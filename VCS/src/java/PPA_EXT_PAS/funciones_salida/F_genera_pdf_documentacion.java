 ...
 */
@WebServlet(name = "F_genera_pdf_documentacion", urlPatterns = {"/F_genera_pdf_documentacion"})
public class F_genera_pdf_documentacion extends HttpServlet {
        ...
        try {
            ...
            if ((carta_comp.size() != 0)) {
                try {
                    ...
                    //Para que se muestre
                    response.setHeader("Content-Disposition", "inline; filename=Documentacion_VCS.pdf");
                    ...
                    Genera_pdf_maestro objTable = new Genera_pdf_maestro();
                    /*File directorio = new File("c:\\VCS\\");//Se guarda el documento en el repositorio local
                     if (!directorio.exists()){
                     directorio.mkdir();
                     }*/

                    objTable.dibujaPdfTodos(document, ruta_imagenes, ls_dimension, carta_comp, relativeWebPathArchivos, cod_carta_comp, institucion, sede);
